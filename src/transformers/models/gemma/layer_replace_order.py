...
class FusedGemmaLayerFunc(torch.autograd.Function):
    ...
    def forward(
        ...
        q_main_q, q_main_scale, q_main_zero_point = compress_pack_quant_zp_base(
            x=q_main, q_bit=q_bit,
            q_method='per-channel', it_num=iteration,
            it_num_thd=iteration_threshold, static_value=static_value['q']
        )
        k_main_q, k_main_scale, k_main_zero_point = compress_pack_quant_zp_base(
    ...
    def backward(ctx, grad_output: torch.Tensor, *args):
        ...
        # TODO: write a fused gelu-hadamard triton kernel
        fn = torch.nn.functional.gelu(gate)
        ...
        q_main = decompression_dequantization_with_zero_point(q_main_q, q_main_scale, q_main_zero_point, ctx.q_bit)
        ...
        k_main = decompression_dequantization_with_zero_point(k_main_q, k_main_scale, k_main_zero_point, ctx.q_bit)
