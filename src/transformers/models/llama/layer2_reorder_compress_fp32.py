...
def lora_forward(w, w_quant_state, w_lora_a, w_lora_b, b, x):
    ...
    x = x_main + x_lora
    return x, x_main, x_lora_a.to(w_lora_a.dtype)


def lora_backward(w, w_quant_state, w_lora_a, w_lora_b, x, x_lora_a, grad_y):
    w_dequant = BF.dequantize_nf4(w, w_quant_state).t().to(grad_y.dtype)
    grad_w_lora_a = x.mT @ (grad_y @ w_lora_b.mT)
    grad_w_lora_b = x_lora_a.mT @ grad_y
    grad_x = grad_y @ w_dequant.T 
    grad_x += (grad_y @ w_lora_b.T @ w_lora_a.T)
...
class FusedLlamaLayerFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor, *args):
        ...
        gate = gate_main + gate_lora_a @ w_gate_lora_b
        del gate_main
        ...
        up = up_main + up_lora_a @ w_up_lora_b
        del up_main
